...
fn i8_array() {
    ...
    let mut builder = ArrayBuilder::list(ArrayBuilder::i8());
    items.serialize(&mut builder).unwrap();

    let ListBuilder { offsets, element } = builder.unwrap_list();
...
fn struct_array() {
    ...
    let mut builder = ArrayBuilder::list(
        ArrayBuilder::r#struct(vec![
            (String::from("a"), ArrayBuilder::i8()),
            (String::from("b"), ArrayBuilder::i8()),
        ])
        .unwrap(),
    );
    items.serialize(&mut builder).unwrap();
    ...
    let I8Builder { buffer: buffer_a } = named_fields[0].1.clone().unwrap_i8();
    let I8Builder { buffer: buffer_b } = named_fields[1].1.clone().unwrap_i8();
